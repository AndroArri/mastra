 ...
 * (e.g. a Railway VM or containerized environment) rather than on the server host.
 * File operations are implemented by shelling out through the sandbox's `executeCommand`,
 * so the agent's file tools and command tools share one VM/container and one view of the repo.
 *
